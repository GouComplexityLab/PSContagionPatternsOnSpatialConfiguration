...
if __name__ == '__main__':
    ...
    p_values = np.arange(0.001, 0.016, 0.001)
    p_values = np.concatenate((p_values, np.asarray([0.1, 0.2, 0.3])))
    ...
    for ICn in np.asarray([1]):
        for p in p_values:
            ...
            ICData_path = './Many corresponding disk localized seeding initial conditions L=' + str(L) + '/'
            ...
            Data_path = './Step2 Cupy Delta12 Data ICn={} N={} Omega={} beta={} nu={} p={:.3f}/'.format(ICn, node_number, Omega, beta, nu, p)
            if not os.path.exists(Data_path):
                print('Data_path:', Data_path)
                raise Exception("The expected Data Path does not exist. Please execute ...")

            # SeriesPath = './Step1 Cupy Delta12 Fig N={} Omega={} beta={} nu={} p={}/'.format(node_number, Omega, beta, nu, p)
            SeriesPath = Data_path.replace("Data", "Figure")
            SeriesPath = SeriesPath.replace("Step2", "Series dataStep2/Series Step2")
            ...
            for iter in range(total_iteration):
                if (iter + 1) % step == 0:
                    ...
                    # # plt.savefig(SeriesPath + 'Urban Susceptible ' + str(iter + 1) + '.png', bbox_inches='tight', pad_inches=0)
                    # plt.savefig(SeriesPath + 'Urban Infectious ' + str(iter + 1) + '.png', bbox_inches='tight', pad_inches=0.5)
                    # plt.close()
                    # # plt.show()

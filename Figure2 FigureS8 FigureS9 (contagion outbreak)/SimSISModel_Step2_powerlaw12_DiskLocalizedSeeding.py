...
if __name__ == '__main__':
    ...
    ####################################################################################################################
    L = 500
    Network_path = './Generated powerlaw Lattice Embedded Networks L=' + str(L) + ' mu=3.6231'
    if not os.path.exists(Network_path):
        raise ('There is not the path of data')
    laplacian_name_npz = '/laplacian_matrix_powerlawLEN L=' + str(L) + ' mu=3.6231_number=1.npz'
    ...
    for ICn in np.asarray([1]):
        for p in p_values:
            ...
            # mat_name = ('CorrespondingLocalizedSeedingICn=' + str(ICn) + 'Omega=' + "{:.3f}".format(Omega).replace('.','dot') + 'SeedingI=' + "{:.3f}".format(SeedingI).replace('.', 'dot') + 'search_k=' + "{:d}".format(search_k).replace('.', 'dot') + '.mat')

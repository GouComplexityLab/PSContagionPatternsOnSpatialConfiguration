...
L = 100;
data_path1 = ['./Generated delta Lattice Embedded Networks L=' num2str(L) ' dg=4'];
data_path2 = ['/mat_matrices'];
mat_name = ['/matrices_deltaLEN_L=' num2str(L) '_dg=4_number=1.mat'];
net_name = ['delta4L' num2str(L)];
show_step = 4;
...
hold on
lw = 1.5; gwcolor = [0.5 0.5 0.5];
plot([0,0],[0,D],'linewidth',lw,'color',gwcolor)
plot([0,D],[D,D],'linewidth',lw,'color',gwcolor)
plot([D,D],[0,D],'linewidth',lw,'color',gwcolor)
plot([D,0],[0,0],'linewidth',lw,'color',gwcolor)
...
for each_one = 1:length(one_indices)
    ...
    h = min(abs(xn-xc),abs(L+xn-xc)) + min(abs(yn-yc),abs(L+yn-yc))

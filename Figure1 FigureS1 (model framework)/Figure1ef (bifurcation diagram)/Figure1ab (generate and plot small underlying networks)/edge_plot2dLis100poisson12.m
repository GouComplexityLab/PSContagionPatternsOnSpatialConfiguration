...
L = 100;
data_path1 = ['./Generated mypoisson Lattice Embedded Networks L=' num2str(L) ' lam=12'];
data_path2 = ['/mat_matrices'];
mat_name = ['/matrices_mypoissonLEN_L=' num2str(L) '_lam=12_number=1.mat'];
net_name = ['poisson12L' num2str(L)];
show_step = 5;
...
for each_one = 1:length(one_indices)
    ...
    h = min(abs(xn-xc),abs(L+xn-xc)) + min(abs(yn-yc),abs(L+yn-yc))/5;

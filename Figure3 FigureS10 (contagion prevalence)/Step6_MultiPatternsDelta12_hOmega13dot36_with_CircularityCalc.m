...
if Omega > 20
    inmain_path = ['/New Step1 Cupy Delta12 Figure N=250000 Omega=' num2str(Omega,'%.1f') ' beta=0.01 nu=0.2'];
end
...
for each = setdiff([1:length(many_Circularity)], [9, 10, 14])
    centroids = many_centroids(each,:);
    [each mod(centroids(1),L), mod(centroids(2),L)]
    Circularity = many_Circularity(each);
    text(4590, mod(centroids(1),L), mod(centroids(2),L), num2str(Circularity,'%.1f'), ...
        'Rotation',68,...

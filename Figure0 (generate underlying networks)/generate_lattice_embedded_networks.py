...
def myplot(ax, x1, y1, x2, y2, L, position_case, color_name, mylinewidth=1.5):
    ...
    if position_case == 1:
        ...
        x02 = x2 - L
        y02 = y2 - L
        y10 = (y01 - y2) / (x01 - x2) * (L - x01) + y01 # 与 x=L 的直线相交于点 (L, y10)
        x10 = (x01 - x2) / (y01 - y2) * (L - y01) + x01 # 与 y=L 的直线相交于点 (x10, L)
        if x10 <= L:
            x00 = x10
            y00 = L
        else:
            x00 = L
            y00 = y10
        y20 = (y02 - y1) / (x02 - x1) * (0 - x02) + y02 # 与 x=0 的直线相交于点 (0, y20)
        x20 = (x02 - x1) / (y02 - y1) * (0 - y02) + x02 # 与 y=0 的直线相交于点 (x20, 0)
        if x20 >= 0:
            x11 = x20
            y11 = 0
        else:
        ...
        x02 = x2 + L
        y02 = y2 + L
        y10 = (y01 - y2) / (x01 - x2) * (0 - x01) + y01 # 与 x=0 的直线相交于点 (0, y10)
        x10 = (x01 - x2) / (y01 - y2) * (0 - y01) + x01 # 与 y=0 的直线相交于点 (x20, 0)
        if x10 >= 0:
            x00 = x10
            y00 = 0
        else:
            x00 = 0
            y00 = y10
        y20 = (y02 - y1) / (x02 - x1) * (L - x02) + y02 # 与 x=0 的直线相交于点 (L, y20)
        x20 = (x02 - x1) / (y02 - y1) * (L - y02) + x02 # 与 y=0 的直线相交于点 (x20, L)
        if x20 <= L:
            x11 = x20
            y11 = L
        else:
        ...
        y02 = y2 + L
        y10 = (y01 - y2) / (x01 - x2) * (L - x01) + y01 # 与 x=L 的直线相交于点 (L, y10)
        x10 = (x01 - x2) / (y01 - y2) * (0 - y01) + x01 # 与 y=0 的直线相交于点 (x10, 0)
        if x10 <= L:
            x00 = x10
            y00 = 0
        else:
            ...
            y00 = y10
        y20 = (y02 - y1) / (x02 - x1) * (0 - x02) + y02 # 与 x=0 的直线相交于点 (0, y20)
        x20 = (x02 - x1) / (y02 - y1) * (L - y02) + x02 # 与 y=L 的直线相交于点 (x20, L)
        if x20 >= 0:
            x11 = x20
            y11 = L
        else:
        ...
        if x10 >= 0:
            ...
            x00 = 0
            y00 = y10
        y20 = (y02 - y1) / (x02 - x1) * (L - x02) + y02 # 与 x=L 的直线相交于点 (L, y20)
        x20 = (x02 - x1) / (y02 - y1) * (0 - y02) + x02 # 与 y=0 的直线相交于点 (x20, 0)
        if x20 <= L:
            x11 = x20
            y11 = 0
...
def generate_many_powerlawLEN(L=100, k=6, mu=2.5, m=3, K=100, numbers=50):
    ...
    while number <= numbers:
        ...
        print('number=', number, "Is G conneted:", nx.is_connected(G))
        if nx.is_connected(G):
            name_mat = 'matrices_mypoissonLEN_L=' + str(L) + '_mu=' + str(mu) + '_number=' + str(number) + '.mat'
            scio.savemat(mat_path + name_mat,
                         {'adjacent_matrix': adjacent_matrix, 'laplacian_matrix': laplacian_matrix})
            print('number=', number, "Is G conneted:", nx.is_connected(G))
            number = number + 1
...
def plot_any_LEN(path, figure_name, adjacent_matrix, L, nodes_position_case='in', text_state=True):
    ...
    for u in np.arange(N, dtype=np.int_):
        ...
        for v in connected_nodes[0]:
            ...
            myplot(ax, x_1, y_1, x_3, y_3, L, position_cases_j_to_each, color_name='black', mylinewidth=0.5)
...
if __name__ == '__main__':
    ...
    # L_used = 50
    # k_used = int(L_used/2)
    # generate_many_deltaLEN(L=L_used, k=k_used, dg=8, numbers=1)
    # generate_many_mypoissonLEN(L=L_used, k=k_used, lam=8, K=100, numbers=1)
    # generate_many_powerlawLEN(L=L_used, k=k_used, mu=3.25494, m=5, K=100, numbers=1)

    L_used = 100
    k_used = int(L_used/2)
    generate_many_deltaLEN(L=L_used, k=k_used, dg=12, numbers=1)
    generate_many_mypoissonLEN(L=L_used, k=k_used, lam=12, K=100, numbers=1)
    generate_many_powerlawLEN(L=L_used, k=k_used, mu=3.62310, m=8, K=100, numbers=1)

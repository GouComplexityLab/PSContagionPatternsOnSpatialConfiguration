...
if __name__ == '__main__':
    ...
    Q_n = 2
    controledOmege = 7.664
    Ds = 3.0; Data_path5 = './Step2 Cupy Powerlaw12 Data IsolateI DecreaseDs/'
    Ds = 16.0; Data_path6 = './Step2 Cupy Powerlaw12 Data IsolateI KeepDs/'
    Data_path_list = [Data_path6,Data_path5]
    min_I0 = 0;
    max_I0 = 45;
    ...
    for Data_path in Data_path_list:
        ...
        im1 = ax.imshow(uncontroledI0.reshape(L, L), cmap=custom_cmap, extent=[0, L, 0, L], clim=(0, 45));  # clim=(0,11)
        ...
        fig, ax = plt.subplots(figsize=(6, 6))
        im1 = ax.imshow(controledI0.reshape(L, L), cmap=custom_cmap, extent=[0, L, 0, L], clim=(min_I0, max_I0));  # clim=(0,11)
        ...
        #     if (iter + 1) % step == 0:
                ...
                fig, ax = plt.subplots(figsize=(6, 6))
                im1 = ax.imshow(I0.reshape(L, L), cmap=custom_cmap, extent=[0, L, 0, L], clim=(min_I0, max_I0));  # clim=(0,11)
                ...
                if sim_time == 4000:
                    ...
                    plt.savefig(NewFigurePath + 'Urban Infectious ' + "{:.0f}".format(sim_time) + ' with colorbar.png', bbox_inches='tight',
                                pad_inches=0)
                    plt.savefig(NewFigurePath + 'Urban Infectious ' + "{:.0f}".format(sim_time) + ' with colorbar.eps', bbox_inches='tight',
                                pad_inches=0)
                    plt.close()
                    # plt.show()
        time_end = time.time()

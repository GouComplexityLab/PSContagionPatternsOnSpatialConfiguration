...
[sol_t, sol_x] = ode45(@(t,x) OdeSISystem(t,x,beta,nu1,gamma), tspan, x0);
endingS = sol_x(end,1);
endingI = sol_x(end,2);
...
set(gca,'XColor',[0.850980392156863 0.325490196078431 0.0980392156862745],'YColor',[0.850980392156863 0.325490196078431 0.0980392156862745],'TickLength',...
...
figure_name = ['./beijing control nu ' 'beta=' num2str(beta,'%.3f') ' nu=' num2str(nu1,'%.3f')  '.eps'];
saveas(gcf, figure_name, 'epsc');

disp(['segreation I = ' num2str(endingI-I)])
disp(['S = ' num2str(S) ', I = ' num2str(I) ', S+I = ' num2str(S+I)])
x0 = [S; I];
[controlnu_t, controlnu_x] = ode45(@(t,x) OdeSISystem(t,x,beta,nu,gamma), tspan, x0);
...
plot(controlnu_t+sol_t(end), controlnu_x(:,2),'LineWidth',gwLineWidth,'LineStyle','-','Color','b');
...
save databeijing_backward_bifurcation_control_nu controlnu_t controlnu_x